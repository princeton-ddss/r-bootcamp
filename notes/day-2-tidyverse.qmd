...
- `stringr::str_detect()` returns `TRUE`/`FALSE` for each string
- `stringr::str_subset()` returns only the matching strings
- `stringr::str_count()` counts matches in each string
...
stringr::str_detect(states, '^New ')
stringr::str_subset(states, 'New')
```

## Transform text with `stringr`

- `stringr::str_to_lower()` and `str_to_upper()` standardize case
- `stringr::str_replace()` and `str_replace_all()` edit matches
- `stringr::str_c()` combines strings
...
tibble(
  original = places,
  lower = stringr::str_to_lower(places),
  slug = stringr::str_replace_all(
    stringr::str_to_lower(places),
    ' ',
    '-'
  ),
  label = stringr::str_c('State: ', places)
)
```
